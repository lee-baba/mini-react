(function (global, factory) {
  ...
  const MuationMask = Placement | Update | ChildDeletion;
  ...
  function childReconciler(shouldTrackEffects) {
      return function reconilcChildFibers(returnFiber, currentFiber, newChild) {
          ...
          if (typeof newChild === "object" && newChild !== null) {
              switch (newChild.$$typeof) {
                  case REACT_ELEMENT_TYPE:
                      return placeSingChild(reconcileSingleElement(returnFiber, currentFiber, newChild));
                  default:
                      // eslint-disable-next-line no-undef
                      {
                          console.warn("");
          ...
          if (typeof newChild === "string" && typeof newChild === "number") {
              return placeSingChild(reconcileSingleTextNode(returnFiber, currentFiber, newChild));
          }
          return null;
      };
  }
  const reconcileChildFiber = childReconciler(true);
  ...
  const appendInitalChild = (parent, child) => {
      parent.appendChild(child);
  };
  const createTextInstance = (content) => {
      return document.createTextNode(content);
  };
  const appendChildToContainer = appendInitalChild;
  ...
  const commitPlacment = (finishedWork) => {
      // eslint-disable-next-line no-undef
      {
  ...
  function scheduleUpdateOnFilber(fiber) {
      // TODO:调度功能
  ...
  function completeUnitOfWork(filber) {
      let node = filber;
      do {
          completeWork(node);
